...
(() => {
  ...
  const discordTitle = (action, data) => {
    const titles = {
      transition_shipment: 'Shipment status updated',
      upsert_package: 'Shipment package updated',
      upsert_customs: 'Customs case updated',
      upsert_consolidation: 'Consolidation batch updated',
      attach_package: 'Package attached to consolidation',
      upsert_invoice: 'Invoice updated',
      record_payment: 'Payment recorded',
      resolve_exception: 'Logistics exception resolved',
      approve_quote: 'Quote approved',
      record_warehouse_movement: 'Warehouse movement recorded',
      upsert_route_leg: 'Shipment route leg updated',
      register_document: 'Shipment document registered',
      upload_document: 'Shipment document uploaded',
    };
    const shipmentId = data?.shipment_id || '';
    return shipmentId ? `${titles[action] || 'Globall Cloud update'} • ${shipmentId}` : (titles[action] || 'Globall Cloud update');
  };

  const discordRoute = (data) => {
    if (data?.from_hub && data?.to_hub) return `${data.from_hub} → ${data.to_hub}`;
    if (data?.origin_hub && data?.destination_hub) return `${data.origin_hub} → ${data.destination_hub}`;
    if (data?.origin && data?.destination) return `${data.origin} → ${data.destination}`;
    return null;
  };

  const discordSeverity = (action, data) => {
    const status = String(data?.to_status || data?.status || '').toLowerCase();
    if (status === 'exception' || status === 'on_hold' || action === 'resolve_exception') return 'high';
    if (action === 'record_payment' || action === 'approve_quote') return 'success';
    return 'info';
  };

  const notifyDiscordForControlPlane = async (action, data, resultBody) => {
    if (!DISCORD_ACTIONS.has(action) || !window.gcSupabase) return;
    try {
      const sessionResult = await window.gcSupabase.auth.getSession();
      const accessToken = sessionResult?.data?.session?.access_token;
      if (!accessToken) return;

      const dedupeBase = JSON.stringify({ action, data: data || {}, result: resultBody?.item?.id || resultBody?.item?.shipment_id || null });
      const key = `gc-discord:${action}:${btoa(unescape(encodeURIComponent(dedupeBase))).slice(0, 180)}`;
      const previous = Number(sessionStorage.getItem(key) || 0);
      if (previous && Date.now() - previous < 5 * 60 * 1000) return;
      sessionStorage.setItem(key, String(Date.now()));

      const shipmentItem = resultBody?.item && typeof resultBody.item === 'object' ? resultBody.item : {};
      const payload = {
        action: 'send',
        server_id: '1515534446122762420',
        event_type: action,
        title: discordTitle(action, data),
        shipment_id: data?.shipment_id || shipmentItem?.shipment_id || shipmentItem?.id || null,
        status: data?.to_status || data?.status || shipmentItem?.operational_status || null,
        route: discordRoute(data),
        severity: discordSeverity(action, data),
        details: [
          data?.package_code ? `Package: ${data.package_code}` : null,
          data?.invoice_number ? `Invoice: ${data.invoice_number}` : null,
          data?.batch_code ? `Batch: ${data.batch_code}` : null,
          data?.document_type ? `Document: ${data.document_type}` : null,
          data?.movement_type ? `Movement: ${data.movement_type}` : null,
        ].filter(Boolean).join(' • ') || null,
      };

      const response = await fetch(DISCORD_FUNCTION, {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        console.warn('[Globall Cloud] Discord bridge:', response.status);
      }
    } catch (error) {
      console.warn('[Globall Cloud] Discord bridge:', error);
    }
  };

  const installDiscordBridge = () => {
    if (window.__gcDiscordBridgeInstalled) return;
    window.__gcDiscordBridgeInstalled = true;
    const nativeFetch = window.fetch.bind(window);

    window.fetch = async (input, init = {}) => {
      const requestUrl = typeof input === 'string' ? input : input?.url || '';
      const method = String(init?.method || (typeof input !== 'string' && input?.method) || 'GET').toUpperCase();
      const isControlPlane = requestUrl.includes('/functions/v1/logistics-control-plane');
      const bodyText = typeof init?.body === 'string' ? init.body : null;
      let requestBody = null;
      let action = null;

      if (isControlPlane && method === 'POST' && bodyText) {
        try {
      ...
      if (isControlPlane && method === 'POST' && action && DISCORD_ACTIONS.has(action) && response.ok) {
        try {
          const cloned = response.clone();
          const body = await cloned.json();
          void notifyDiscordForControlPlane(action, requestBody?.data || requestBody || {}, body);
        } catch {
          // Never block or alter the original control-plane response.
        }
      }
      return response;
    };
  ...
  const hideLegacyNoticeImmediately = () => {
  ...
  window.gcDiscordNotify = notifyDiscordForControlPlane;
