...
(() => {
  ...
  // Only legacy handlers that are present in reviewed first-party markup may run.
  // This prevents an injected data-gc-on* attribute from invoking arbitrary window APIs.
  const allowedActions = new Set([
    'calcQuote', 'cancelDeleteCustomer', 'cancelDeleteDirectoryEntry', 'checkCapsLock',
    'closeChangelog', 'closeCustomerReceipts', 'closeDirectoryForm', 'doDeleteCustomer',
    'doDeleteDirectoryEntry', 'enableNotifications', 'goTrack', 'handleContactSubmit',
    'handleCustomerSignIn', 'handleCustomerSignUp', 'handleRequestSubmit', 'handleSignOut',
    'handleStaffSignIn', 'handleStaffSignOut', 'handleVerifySignupCode', 'mfaVerifyChallenge',
    'openChangelog', 'openMfaSettings', 'pbnTap', 'portalTrackGo', 'resendSignupCode',
    'route', 'runQuickQuote', 'saveDirectoryEntry', 'setLang', 'setPortalAuthTab',
    'setPortalTab', 'toggleMobileMenu', 'togglePwVisibility', 'toggleTheme',
  ]);
  const decode = (value) => {
  ...
  const call = (name, args, event, element) => {
    if (!allowedActions.has(name)) return false;
