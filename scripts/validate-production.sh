...
check "staff clean routes and auth bridge" bash -c 'grep -q "^/staff-os /staff-os.html 200$" _redirects && grep -q "^/staff-os/ /staff-os.html 200$" _redirects && grep -q "staff-auth-fix.js" functions/_middleware.js && grep -qF "staff(?:-os)?" staff-auth-fix.js'
