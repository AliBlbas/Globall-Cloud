...
check "service worker contains control-plane, customer, payment and tracking assets" bash -c 'grep -q "control-plane.html" sw.js && grep -q "control-plane.js" sw.js && grep -q "customer-portal.html" sw.js && grep -q "payment-checkout.html" sw.js && grep -q "payment-checkout.js" sw.js && grep -q "tracking-route-controller.js?v=20260826-1" sw.js && grep -q "gc-v71" sw.js'
...
check "staff routes and auth bridge" bash -c 'grep -q "^/staff /staff-os.html 200$" _redirects && grep -q "^/staff/ /staff-os.html 200$" _redirects && grep -q "staff-auth-fix.js" functions/_middleware.js && grep -qF "staff(?:-os)?" staff-auth-fix.js'
  check "analytics bridge, read-only data hub, and staff auth assets are wired and cached" bash -c 'grep -q "site-analytics.js?v=20260824-1" index.html && grep -q "site-analytics.js?v=20260824-1" sw.js && test -f site-analytics.js && grep -q "control-plane-data-hub.js?v=20260825-1" control-plane.html && grep -q "control-plane-data-hub.js?v=20260825-1" sw.js && test -f control-plane-data-hub.js && grep -q "staff-auth-fix.js?v=20260825-7" sw.js && grep -q "staff-os-console.js?v=20260825-14" sw.js && grep -q "staff-os-ultra.css?v=20260825-11" sw.js && grep -q "staff-os-ultra.css?v=20260825-11" staff-os.html && grep -q "staff-os-premium.css?v=20260825-2" sw.js && grep -q "customer-portal-inline-1.js?v=20260825-2" sw.js && grep -q "customer-portal-inline-1.js?v=20260825-2" customer-portal.html && test -f staff-auth-fix.js && test -f staff-os-console.js && test -f gc-csp-scripts/customer-portal-inline-1.js'
...
check "standalone tracking route is wired and script-safe" bash -c 'test -f tracking-route-controller.js && grep -q "id=\"gcTrackingForm\"" tracking-integration.html && grep -q "tracking-route-controller.js?v=20260826-1" tracking-integration.html && ! grep -qE "<script[^>]*>([^<]|$)" tracking-integration.html && ! grep -qiE "\\s(on(click|change|submit|keydown|keyup|input|load|error|focus|blur|mouseover|mouseout))\\s*=" tracking-integration.html'
