...
check "service worker contains control-plane, customer and payment assets" bash -c 'grep -q "control-plane.html" sw.js && grep -q "control-plane.js" sw.js && grep -q "customer-portal.html" sw.js && grep -q "payment-checkout.html" sw.js && grep -q "payment-checkout.js" sw.js && grep -q "gc-v39" sw.js'
check "Supabase function is JWT-protected" bash -c 'grep -q "\[functions.logistics-control-plane\]" supabase/config.toml && grep -q "verify_jwt = true" supabase/config.toml'
