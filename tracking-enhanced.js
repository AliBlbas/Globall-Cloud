...
class EnhancedTracking {
    ...
    return `
      <div class="live-map-container">
        <svg viewBox="0 0 1000 600" class="route-visualization">
          ...
          <text x="150" y="330" text-anchor="middle" class="map-label">${(typeof placeLabel==='function'?placeLabel(shipment.origin_key):null) || shipment.origin_key || '—'}</text>
          ...
          <polyline points="150,300 350,285 500,270 700,255 850,300"
                    fill="none" class="map-route-path" stroke-dasharray="5,5"/>

          <circle cx="850" cy="300" r="15" class="map-point dest-point"/>
          <text x="850" y="330" text-anchor="middle" class="map-label">${(typeof placeLabel==='function'?placeLabel(shipment.dest_key):null) || shipment.dest_key || '—'}</text>
        </svg>
  ...
  // NOTE (integration fix): originally gated on an element id
  // (`tracking-${shipmentId}`) that no page ever creates, so realtime
  // updates would silently never repaint the map after the first render.
  // liveMapContainer is the one real container index.html creates, so we
  // just re-render straight into it.
  updateTrackingUI(shipmentId) {
    if (!document.getElementById('liveMapContainer')) return;
