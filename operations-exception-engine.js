 ...
 * Uses the publishable Supabase client only; all privileged writes stay behind RLS/RPC.
 */
(() => {
  'use strict';
  ...
  function exceptionCard(row) {
    ...
    return `<div class="${cls}">
      <strong>${esc(row.title || 'Shipment exception')}</strong>
      <div class="muted">${esc(row.description || row.reason || 'Requires operational review.')}</div>
      <div style="margin-top:6px;font-size:11px" class="mono">${esc(row.shipment_id || '—')} · ${esc(row.status || 'open')} · ${esc(severity)}</div>
    </div>`;
  }

  async function loadExceptions() {
    const { data, error } = await state.client.rpc('get_staff_exception_queue');
  ...
  async function loadShipments() {
    if (!state.user) {
      set('kShip', '—'); set('kMoving','—'); set('kDue','—');
      return empty('shipments', 'چوونەژوورەوە بکە بۆ بینینی shipment ـەکان.');
    }
    ...
    set('kShip', state.shipments.length); set('kMoving', moving); set('kDue', money(due));
    renderShipments();
  }
  ...
  function renderShipments() {
    ...
    let rows = state.shipments.filter(s => {
    ...
    document.querySelectorAll('[data-track-id]').forEach(btn => btn.addEventListener('click', () => {
      const id = btn.getAttribute('data-track-id');
      window.location.href = `./index.html?tracking=${encodeURIComponent(id)}#track`;
    }));
  }
  ...
  async function refreshAll() {
    try {
      ...
      if (state.user) {
        ...
        set('who', esc(who));
        set('rolePill', esc(state.isStaff ? (staff.data?.role || 'Staff') : 'Customer'));
        set('sessionText', state.isStaff ? 'Staff session ـی پشتڕاستکراو.' : 'Customer session ـی پشتڕاستکراو.');
      } else {
        state.isStaff = false;
        set('rolePill','Guest'); set('who','Guest');
      }
      ...
      console.error('[Globall Cloud] Exception engine:', error);
      set('dataState','<span class="pill bad">Data Error</span>');
    }
  }
