-- v4 notification worker hardening.
-- The staff console is allowed to process only in-app rows. External channels
-- are reserved for the server-only notification-dispatch worker.
...
is 'Claims one notification channel for a trusted worker; staff UI must use in_app only.';
