-- Restore the shipment transition contract that the Staff Control Plane calls.
-- This is intentionally shipment-specific; shopping_orders keeps its own status validator.
...
begin
  ...
  if v_status in ('pending','booked') then
    ...
    if v_from_step > 5 then raise exception 'Invalid shipment step'; end if;
  else
...
-- Normalize the one existing real shipment whose status was already in_transit
-- but whose step index was still 0; do not invent any customer or transport data.
update public.shipments
set current_step_index = 1
where operational_status = 'in_transit'
