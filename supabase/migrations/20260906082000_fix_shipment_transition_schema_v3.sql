-- Align the shipment transition contract with the actual production schema.
-- shipments.id is UUID and there is no state_version column.
...
begin
  ...
  insert into public.shipment_events(
    ...
    jsonb_build_object('to_step', v_step, 'from_step', v_from_step, 'metadata', coalesce(p_metadata,'{}'::jsonb))
  );
  ...
  if v_shipment.customer_user_id is not null and to_regprocedure('public.enqueue_customer_notification(uuid,text,text,text,text,text,jsonb)') is not null then
    v_event_key := format('shipment:%s:status:%s:%s', p_shipment_id, v_status, to_char(v_now, 'YYYYMMDDHH24MISSMS'));
