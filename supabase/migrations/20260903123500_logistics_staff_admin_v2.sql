...
as $$
begin
  if not exists(select 1 from public.staff where id=p_actor_id and is_active=true and role in ('admin','super_admin')) then
    raise exception 'Staff admin permission required';
  end if;
  return query
  select
    s.id,s.full_name,s.role,s.branch,s.is_active,s.created_at,s.updated_at,
    au.email,au.phone,
