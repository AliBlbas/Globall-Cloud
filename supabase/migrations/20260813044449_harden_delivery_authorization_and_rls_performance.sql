...
revoke execute on function public.advance_delivery_assignment(uuid,text,text) from anon;
revoke execute on function public.complete_delivery(text,text,text,text,numeric,numeric,jsonb,text) from anon;
...
alter policy "shipment_financial_ledger_staff_select" on public.shipment_financial_ledger using ((select auth.uid()) is not null and exists(select 1 from public.staff s where s.id=(select auth.uid()) and s.is_active=true));
alter policy "shipment_financial_ledger_staff_insert" on public.shipment_financial_ledger with check ((select auth.uid()) is not null and exists(select 1 from public.staff s where s.id=(select auth.uid()) and s.is_active=true));;
