...
const build = async (service: ReturnType<typeof serviceClient>, staff: Staff) => {
  ...
  const totalBilled = finance ? invoices.reduce((sum, row) => sum + Number(row.total_amount || 0), 0) : null
  const successfulPayments = finance ? payments.filter(p => String(p.status || '').toLowerCase() === 'succeeded').reduce((sum, row) => sum + Number(row.amount || 0), 0) : null
