...
import { requireServiceKey } from '../_shared/service-key.ts'
...
async function auth(req:Request){
  ...
  if(!url||!anon||!/^Bearer\s+/i.test(authorization)) throw new Error('Unauthorized')
...
async function dashboard(a:{admin:any,staff:Staff}){
  ...
  const [exceptionsResult,unverifiedResult,queueResult]=await Promise.all([
    ...
    admin.from('shipment_documents').select('id',{count:'exact',head:true}).neq('document_status','verified'),
    admin.from('notification_outbox').select('id',{count:'exact',head:true}).in('status',['queued','processing','retry']),
  ])
...
async function updateException(a:{admin:any,staff:Staff},data:any){
  ...
  const {data:ex,error}=await a.admin.from('logistics_exceptions').select('id,shipment_id,status,severity,title').eq('id',id).maybeSingle()
  ...
  if(ex.shipment_id && a.staff.branch && a.staff.branch!=='all' && !GLOBAL_ROLES.has(a.staff.role)){
    const {data:shipment,error:shipmentError}=await a.admin.from('shipments').select('branch').eq('id',ex.shipment_id).maybeSingle()
    if(shipmentError) throw shipmentError
    if(String(shipment?.branch||'') && String(shipment.branch)!==String(a.staff.branch)) throw new Error('Branch access denied')
  } else if(!ex.shipment_id && !GLOBAL_ROLES.has(a.staff.role)) {
    throw new Error('Global exception requires elevated role')
  }
  ...
  const {data:updated,error:updateError}=await a.admin.from('logistics_exceptions').update(patch).eq('id',id).select('*').single()
  if(updateError) throw updateError

  const {error:activityError}=await a.admin.from('staff_activity_log').insert({staff_id:a.staff.id,staff_name:a.staff.full_name,action:'logistics.exception.update',target_id:ex.shipment_id||id,details:JSON.stringify({exception_id:id,status,resolution_note:note||null})})
...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    const status=/Unauthorized/i.test(raw)?401:/Forbidden|denied/i.test(raw)?403:/not found|Invalid|Unsupported/i.test(raw)?400:500
    console.error('logistics-control-tower request failed',{status,message:raw})
    const message=status===401?'Unauthorized':status===403?'Forbidden':status===400?raw:'Internal server error'
