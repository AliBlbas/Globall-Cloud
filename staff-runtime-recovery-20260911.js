(() => {
  ...
  window.addEventListener('error', e => {
    ...
    if (/staff|supabase|syntax|undefined|null|is not a function|failed to load/i.test(String(msg))) setTimeout(() => showRecovery(String(msg)), 50);
  }, true);
  ...
  const probe = () => {
    if (shown) return;
    const shell = document.querySelector('.gc-shell, .login');
    const app = document.getElementById('app');
    if (shell) return;
    if (Date.now() - started > 12000 && app && !app.innerHTML.trim()) showRecovery('Staff UI ـەکە بار نەکرا.');
  };
  const timer = setInterval(() => { probe(); if (Date.now() - started > 30000) clearInterval(timer); }, 1000);
