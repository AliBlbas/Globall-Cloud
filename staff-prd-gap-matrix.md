...
| --- | --- | --- | --- |
| Staff authentication and role gate | Working Staff OS login, protected account-admin API, owner role/branch visible | No user-facing CRUD in Staff OS; legacy Super Admin surface is partially separate | Consolidate staff management through protected account-admin actions and add explicit create/edit/activate controls |
...
| Warehouse receipts | account-admin receipt list/create path exists; multipart photo upload currently targets `warehouse-receipts` | PRD requests cargo-linked receipt table, multiple evidence photos, GPS, lightbox, edit/delete, and audit trail; current Staff OS is list-only | Extend the existing protected receipt API with validated metadata/photo operations; confirm bucket/policies before UI writes |
